...
namespace MediaBazaarApplicationWPF
{
    class Attendance
    {
        ...
        {
            get => this.attendanceRate;
            set { this.attendanceRate = value; }
        }
            ...
            else
            {
                using (MySqlConnection conn = SqlConnectionHandler.GetSqlConnection())
                {
                    using (MySqlCommand cmd = new MySqlCommand(@"SELECT attendance FROM employees WHERE userID=@userID", conn))
                    {
                        ...
                        if (dataReader.Read())
                        {
                            if (!dataReader.IsDBNull(0)) this.AttendanceRate = Convert.ToDouble(dataReader.GetDecimal(0));
                        }
                        cmd.Dispose();
                        dataReader.Close();
                    }
        ...
        public override string ToString() => $"{attendanceRate:F2}%";

        public static void UpdateAllAttendance()
        {
            foreach (Employee e in EmployeeDatabaseHandler.GetAllEmployees(false)) { Attendance newAttendance = new Attendance(e.UserID, true); }
        }
    }
}
